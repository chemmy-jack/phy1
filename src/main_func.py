...
#		wb_trail.start()
	...
	global o_wb, o_wt, o_te, o_ta, wt, te, ta, o_wt_r, o_te_r, o_ta_r
	def initialize() :
		global o_wb, o_wt, o_te, o_ta, wt, te, ta, o_wt_r, o_te_r, o_ta_r
		m = DataMenu
		...
		# clear trail
		# cleartrail_func()
		# clear clones (after image)
		aftimgbut()
		spec_data = alldata[m.selected]
		origin_coordinate = func.cal_origin_coordinate(spec_data)
		print("ocoordinate: ", origin_coordinate.keys()
		oo_wb = origin_coordinate["wb"]
		oo_wt = origin_coordinate["wt"]
		oo_te = origin_coordinate["te"]
		oo_ta = origin_coordinate["ta"]
	...
	# start running visualized graph
	print("the while loop is ruuuuuuning")
	while True :
		sleep(dt)
		if keepon :
			print("should update")
